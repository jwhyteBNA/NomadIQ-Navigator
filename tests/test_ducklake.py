...
def test_ducklake_setup_sql_calls():
    class DummyConn:
        def __init__(self):
            self.calls = []
...
def test_ducklake_setup_with_invalid_path(tmp_path, monkeypatch):
    db_path = tmp_path / "test_ducklake_setup_with_invalid_path.db"
    original_connect = duckdb.connect
    monkeypatch.setattr("duckdb.connect", lambda database='ducklake.db': original_connect(str(db_path)))
    conn = duckdb_setup()
